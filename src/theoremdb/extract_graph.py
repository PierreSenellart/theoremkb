...
from .features import process_paper
...
def save_graph(name,df_out_res,df_out_links):
	...
	df_out_res.to_csv("graph_results_%s.csv"%name,index=False,header=["pdf_from","result"])

	df_out_links = pd.DataFrame(df_out_links)
	df_out_links.to_csv("graph_intra_extra_%s.csv"%name,index=False,header=["pdf_from","nres_in","theorem_in","theorem_ref","intra","ref_tag","pdf_to"])
...
def extract_graph(name,multithreading=True,n_jobs=4,chunks_size=1000):
	...
	thmdb = TheoremDB(list_paper=['1804.01710'],merge_all=True)
